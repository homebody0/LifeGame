...
Field::Field(sf::Vector2u size) {
    mFieldImage = new sf::Image();

    mImageSize = new sf::Vector2u;
    *mImageSize = mFieldImage->getSize();

    mRenderSize = new sf::Vector2u;
    *mRenderSize = size;

    mFieldTexture = new sf::Texture;
    mFieldTexture->loadFromImage(*mFieldImage);
...
void Field::updateCells() {
    ...
    for (unsigned int x = 0; x < mImageSize->x; ++x) {
        for (unsigned int y = 0; y < mImageSize->y; ++y) {
            sf::Color previousState = mFieldImage->getPixel(x, y);
    ...
    *mFieldImage = newField;
}

void Field::draw(sf::RenderTarget& target, sf::RenderStates states) const {
    mFieldTexture->update(*mFieldImage);
        ...
        for (int j = -1; j < 2; ++j) {
            if (i != 0 or j != 0) {
                try {
                    ...
                    if (mFieldImage->getPixel(x + i, y + j) == sf::Color::White) {
                        ++sum;
                    }
                } catch(...) {
                    continue;
...
void Field::save(std::string &filename) {
    mFieldImage->saveToFile(filename);
}

void Field::upload(const std::string &filename) {
    mFieldImage->loadFromFile(filename);
    mFieldTexture->update(*mFieldImage);
}

void Field::setPixel(sf::Vector2u position, sf::Color color) {
    mFieldImage->setPixel(position.x, position.y, color);
}

sf::Vector2u Field::getSizeField() {
    return mFieldImage->getSize();
}
...
//        mFieldImage->setPixel(float(mousePosition.x) / mRenderSize.x * mImageSize.x,
...
//    {
//        mFieldImage->saveToFile("..\\out.png");
//    }
//
